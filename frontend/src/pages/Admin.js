...
const Admin = (props) => {
  ...
  const sendMessage = () => {
    ...
    setMessages((messages) => [
      ...messages,
      { message: newMessage.message, sender: "Me" },
    ]);
    let mistabs = [...tabs];
  ...
  const scrollToBottom = () => {
    if (Object.keys(commentsEndRef.current).length) {
  ...
  console.log(tabs);
  if (!token && !admin) {
    return <p>NO ESTAS AUTORIZADO A VER ESTO</p>;
  }
  return (
        ...
        <div className="whoImHelpingContainer">
          ...
          {users.find((user) => user.id === willHelp.whoToHelp) && (
            <div className="userOnLine">
              <p>
                Nombre:
                {users.find((user) => user.id === willHelp.whoToHelp).firstName}
              </p>
              <p>
                Email:
                {users.find((user) => user.id === willHelp.whoToHelp).eMail}
              </p>
              <p>
                Id:{users.find((user) => user.id === willHelp.whoToHelp).id}
              </p>
            </div>
          )}
        </div>
      ...
      <div className="sendContainerChat">
        <div className="handleWhoToHelpContainer">
          ...
          <input
            ...
            type="text"
            name="sendTo"
            value={newMessage.sendTo}
            disabled
          ></input>
          <label htmlFor="whoToHelp">Habilitar chat a: </label>
          <input
            onChange={inputHelpHandler}
            type="text"
            name="whoToHelp"
            value={willHelp.whoToHelp}
          ></input>
